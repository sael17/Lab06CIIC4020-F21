...
//            Node next = rmNode.getNext();
        ...
        public  void reverse() {
            if (isEmpty()) return;
            Node current = header.getNext(); // starting node
            Node prev = null; // previous node, this will be our head at the end
            Node next = null; // save value for next iteration
            while (current != trailer) {
                next = current.getNext(); // save value for next iteration
                current.next = prev; // reverse items
                current.prev = next; // reverse items
                prev = current; // our new previous is the current node
                current = next; // advanced in the linkedlist
            }
